...
/*
 ...
 注意
 Swift 中的错误处理和其他语言中用try，catch和throw进行异常处理很像。和其他语言中（包括 Objective-C ）的异常处理不同的是，Swift 中的错误处理并不涉及解除调用栈，这是一[…]”
...
class VendingMachine {
    ...
    func vend(itmeNamed name: String) throws {
...
func buyFavoriteSnack(person: String, vendingMachine: VendingMachine) throws {
    ...
    try vendingMachine.vend(itmeNamed: snackName)
}
...
struct PurchasedSnack {
    ...
    init(name: String, vendingMachine: VendingMachine) throws {
        try vendingMachine.vend(itmeNamed: name)
        self.name = name
    }
}
...
/*
 ...
 例如，下面的代码使用了loadImage(atPath:)函数，该函数从给定的路径加载图片资源，如果图片无法载入则抛出一个错误。在这种”
 
 “情况下，因为图片是和应用绑定的，运行时不会有错误抛出，所以适合禁用错误传递：
